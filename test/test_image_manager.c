...
static void test_load_and_query(void)
{
    ...
    CHECK(id != NULL && id[0] != '\0', "id should be set");
    ...
    /* find by id */
    image found = image_manager_find_by_id(mgr, id);
    CHECK(found == img, "find_by_id should return same image");
    ...
    /* not found cases */
    found = image_manager_find_by_id(mgr, "0000000000000000");
    CHECK(found == NULL, "find nonexistent id should return NULL");
...
{
    printf("  test_duplicate_id_rejected... ");
    ...
    /* loading same sqfs again should fail (same id) */
    image img2 = image_manager_load(mgr, sqfs_path);
    CHECK(img2 == NULL, "duplicate id should be rejected");
...
static void test_remove(void)
{
    ...
    image img = image_manager_load(mgr, sqfs_path);
    CHECK(img != NULL, "load image");
    char *id = strdup(image_get_id(img));
...
static void test_persistence(void)
{
    ...
    /* first session: load image, leave mounted */
    {
        ...
        image img = image_manager_load(mgr, sqfs_path);
        CHECK(img != NULL, "load image");
        saved_id = strdup(image_get_id(img));
    ...
    /* second session: image should be rediscovered and still mounted */
    {
        ...
        image found = image_manager_find_by_id(mgr, saved_id);
    ...
    /* third session: after umount_all, image should exist but be unmounted */
    {
        ...
        image found2 = image_manager_find_by_id(mgr, saved_id);
